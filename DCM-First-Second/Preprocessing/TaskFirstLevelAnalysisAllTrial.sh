...
cp $Templatefsf/design.fsf $FEATDir/design.fsf
...
sed -i -e "s|ProLeftFSL.txt|${FEATDir}/${subj}${TimingDesign}ProLeft.txt|g" ${FEATDir}/design.fsf

sed -i -e "s|ProRightFSL.txt|${FEATDir}/${subj}${TimingDesign}ProRight.txt|g" ${FEATDir}/design.fsf

sed -i -e "s|AntiLeftFSL.txt|${FEATDir}/${subj}${TimingDesign}AntiLeft.txt|g" ${FEATDir}/design.fsf

sed -i -e "s|AntiRightFSL.txt|${FEATDir}/${subj}${TimingDesign}AntiRight.txt|g" ${FEATDir}/design.fsf
...
DesignFContrast=${FEATDir}/design.fts

film_gls --rn=${FEATDir}/stats --sa --ms=5 --in=$fMRI/${subject}/fMRI$n/${input}.nii.gz --pd=${DesignMatrix} --con=${DesignTContrast} --fcon=${DesignFContrast} --thr=0 
